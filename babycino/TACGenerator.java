...
public class TACGenerator extends MiniJavaBaseVisitor<TACBlock> {
    ...
    public TACBlock visitExpBinOp(MiniJavaParser.ExpBinOpContext ctx) {
        ...
        if (op.equals(">=")) {
            // && should short-circuit.
            String res1 = this.genreg();
            String res2 = this.genreg();
			String resultreg = this.genreg();
			String isTrue = this.genreg();
			String mid = this.genlab();
			String end = this.genlab();
			String t2 = this.genreg();
			String isFalse = this.genlab();

            result.addAll(expr1);
            result.add(TACOp.mov(res1, expr1.getResult()));
            result.addAll(expr2);
            result.add(TACOp.mov(res2, expr2.getResult()));

			result.add(TACOp.binop(resultreg, res2, res1, 0));
			result.add(TACOp.jz(resultreg,mid));
			result.add(TACOp.jmp(isTrue));
			result.add(TACOp.label(mid));
			result.add(TACOp.binop(t2,res2,res1,2));
			result.add(TACOp.jz(t2, isTrue));
			result.add(TACOp.jmp(isFalse));
			result.add(TACOp.label(isFalse));
			result.add(TACOp.immed(resultreg,0));
			result.add(TACOp.jmp(end));
			result.add(TACOp.label(isTrue));
